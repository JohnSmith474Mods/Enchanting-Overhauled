...
public class EnchantmentScreen extends AbstractContainerScreen<EnchantmentMenu> {
    ...
    protected void renderBg(GuiGraphics context, float partialTick, int mouseX, int mouseY) {
        ...
        if (!this.menu.getSlot(0).hasItem()) {
            context.blit(RenderPipelines.GUI_TEXTURED, EMPTY_SLOT_TARGET_TEXTURE, alignX + this.menu.getSlot(0).x, alignY + this.menu.getSlot(0).y, 0, 0, 16, 16, 16, 16);
        }
        if (!this.menu.getSlot(1).hasItem()) {
            context.blit(RenderPipelines.GUI_TEXTURED, EMPTY_SLOT_FUEL_TEXTURE, alignX + this.menu.getSlot(1).x, alignY + this.menu.getSlot(1).y, 0, 0, 16, 16, 16, 16);
        }
        if (!this.menu.getSlot(2).hasItem()) {
            context.blit(RenderPipelines.GUI_TEXTURED, EMPTY_SLOT_SOURCE_TEXTURE, alignX + this.menu.getSlot(2).x, alignY + this.menu.getSlot(2).y, 0, 0, 16, 16, 16, 16);
        }
        ...
        for (int visualIndex = 0; visualIndex < PAGE_SIZE; ++visualIndex) {
            ...
            if (id >= 0 && this.minecraft.level != null) {
                IdMap<Holder<Enchantment>> idMap = this.minecraft.level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).asHolderIdMap();
    ...
    private void drawNavButtons(GuiGraphics context, int alignX, int alignY, int mouseX, int mouseY, int totalEnchants) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, prevTex, prevX, prevY, 0, 0, BACKWARD_BUTTON_WIDTH, BACKWARD_BUTTON_HEIGHT, BACKWARD_BUTTON_WIDTH, BACKWARD_BUTTON_HEIGHT);
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, nextTex, nextX, nextY, 0, 0, FORWARD_BUTTON_WIDTH, FORWARD_BUTTON_HEIGHT, FORWARD_BUTTON_WIDTH, FORWARD_BUTTON_HEIGHT);
    }
    // endregion
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
        ...
        if (totalEnchants > PAGE_SIZE) {
            // Backward Click
            if (this.isHovering(BACKWARD_X_OFFSET, BACKWARD_Y_OFFSET, BACKWARD_BUTTON_WIDTH, BACKWARD_BUTTON_HEIGHT, event.x(), event.y())) {
                if (navigatePage(hasTargetItem, totalEnchants, -1)) return true;
            }
            // Forward Click
            if (this.isHovering(FORWARD_X_OFFSET, FORWARD_Y_OFFSET, FORWARD_BUTTON_WIDTH, FORWARD_BUTTON_HEIGHT, event.x(), event.y())) {
                if (navigatePage(hasTargetItem, totalEnchants, 1)) return true;
            }
        }
    ...
    private void drawEmptySlot(GuiGraphics context, int buttonX, int buttonY, int texIndex, boolean usePlain) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, disabledTex, buttonX, buttonY, 0, 0, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT);
    }
    ...
    private void drawEnchantmentSlot(
        ...
        if (!affordable) {
            context.blit(RenderPipelines.GUI_TEXTURED, disabledTex, buttonX, buttonY, 0, 0, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT);
            context.blitSprite(RenderPipelines.GUI_TEXTURED, DISABLED_LEVEL_SPRITES[costIndex], buttonX + ENCHANTING_COST_X_OFFSET, buttonY + ENCHANTING_COST_Y_OFFSET, ENCHANTING_COST_WIDTH, ENCHANTING_COST_HEIGHT);
        } else {
            ResourceLocation textureToDraw = hovering ? highlightedTex : enabledTex;
            context.blit(RenderPipelines.GUI_TEXTURED, textureToDraw, buttonX, buttonY, 0, 0, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT);
            context.blitSprite(RenderPipelines.GUI_TEXTURED, ENABLED_LEVEL_SPRITES[costIndex], buttonX + ENCHANTING_COST_X_OFFSET, buttonY + ENCHANTING_COST_Y_OFFSET, ENCHANTING_COST_WIDTH, ENCHANTING_COST_HEIGHT);
        }
        ...
        // MutableComponent outlineComponent = displayComponent.copy();
        // outlineComponent.withStyle(style -> style.withColor(outlineColor));
        // context.drawWordWrap(this.font, outlineComponent, textX + 1, textY, ENCHANTING_TEXT_MAX_WIDTH, outlineColor, false);
        // context.drawWordWrap(this.font, outlineComponent, textX - 1, textY, ENCHANTING_TEXT_MAX_WIDTH, outlineColor, false);
        // context.drawWordWrap(this.font, outlineComponent, textX, textY + 1, ENCHANTING_TEXT_MAX_WIDTH, outlineColor, false);
        // context.drawWordWrap(this.font, outlineComponent, textX, textY - 1, ENCHANTING_TEXT_MAX_WIDTH, outlineColor, false);
        context.drawWordWrap(this.font, displayComponent, textX, textY, ENCHANTING_TEXT_MAX_WIDTH, targetTextColor, false);
    ...
    private void drawMaxedSlot(GuiGraphics context, Holder<Enchantment> enchantment, int enchantmentLevel, int buttonX, int buttonY) {
        ...
        // 2. Render Background
        context.blit(RenderPipelines.GUI_TEXTURED, backgroundTexture, buttonX, buttonY, 0, 0, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT, ENCHANTING_BUTTON_WIDTH, ENCHANTING_BUTTON_HEIGHT);
    ...
    private void drawRerollButton(GuiGraphics context, int alignX, int alignY, int mouseX, int mouseY, int lapisCount, int[] enchantmentSources) {
        ...
        if (rerollEnabled) {
            ...
            if (mouseXdiff >= 0 && mouseYdiff >= 0 && mouseXdiff < REROLL_BUTTON_WIDTH && mouseYdiff < REROLL_BUTTON_HEIGHT) {
                context.blit(RenderPipelines.GUI_TEXTURED, REROLL_HIGHLIGHTED_TEXTURE, x, y, 0, 0, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT);
            } else {
                context.blit(RenderPipelines.GUI_TEXTURED, REROLL_TEXTURE, x, y, 0, 0, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT);
            }
            if (costIndex < ENABLED_LEVEL_SPRITES.length) {
                context.blitSprite(RenderPipelines.GUI_TEXTURED, ENABLED_LEVEL_SPRITES[costIndex], x + REROLL_COST_X_OFFSET, y + REROLL_COST_Y_OFFSET, REROLL_COST_WIDTH, REROLL_COST_HEIGHT);
            }
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, REROLL_DISABLED_TEXTURE, x, y, 0, 0, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT, REROLL_BUTTON_WIDTH, REROLL_BUTTON_HEIGHT);
            ...
            if (showDisabledCost && costIndex < DISABLED_LEVEL_SPRITES.length) {
                context.blitSprite(RenderPipelines.GUI_TEXTURED, DISABLED_LEVEL_SPRITES[costIndex], x + REROLL_COST_X_OFFSET, y + REROLL_COST_Y_OFFSET, REROLL_COST_WIDTH, REROLL_COST_HEIGHT);
            }
        }
    }
    ...
    private void drawApplyTooltip(GuiGraphics context, int mouseX, int mouseY, int lapisCount, boolean isCreative, Holder<Enchantment> enchantment, int enchantmentLevel, int powerRequirement) {
        ...
        if (registryOpt.isPresent()) {
            Optional<Holder.Reference<EnchantmentTheme>> optional = registryOpt.get().get(themeKey);
            if (optional.isPresent()) {
                EnchantmentTheme theme = optional.get().value();
